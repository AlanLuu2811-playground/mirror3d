...
# Clone from "https://github.com/NVIDIA/TensorRT/blob/release/8.6/samples/python/detectron2/create_onnx.py#L144"
...
sys.path.append(f"{os.environ['HOME']}/dev/mirror3d/mirror3d/mirror3dnet")
...
class DET2GraphSurgeon:
    def __init__(self, saved_model_path, config_file, weights, data_dir):
        """
        ...
        def det2_setup(config_file):
            ...
            # MODEL
            cfg.ANCHOR_NORMAL_NYP = f'{data_dir}/kmeans_normal_10.npy'
    ...
    def get_anchors(self, sample_image):
        ...
        model = Mirror3dTrainer.build_model(self.det2_cfg)
        ...
        imagelist_images = ImageList.from_tensors(images, model.backbone.size_divisibility)
    ...
    def NMS(self, boxes, scores, anchors, background_class, score_activation, max_proposals, iou_threshold, nms_score_threshold, user_threshold, nms_name=None):
        ...
        nms_output_classes = gs.Variable(name="detection_classes"+nms_name, dtype=np.int32,
                                         shape=[self.batch_size, max_proposals])
        nms_output_scores_2 = gs.Variable(name="detection_scores_2"+nms_name, dtype=np.float32,
                                        shape=[self.batch_size, max_proposals])
        nms_output_classes_2 = gs.Variable(name="detection_classes_2"+nms_name, dtype=np.int32,
                                         shape=[self.batch_size, max_proposals])

        #nms_outputs = [nms_output_num_detections, nms_output_boxes, nms_output_scores, nms_output_classes, nms_output_scores_2, nms_output_classes_2]
...
def main(args):
    det2_gs = DET2GraphSurgeon(args.exported_onnx, args.det2_config, args.det2_weights, args.data_dir)
